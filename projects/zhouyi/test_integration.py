    ...
    try:
        ...
        for hex_id in sample_ids:
            ...
            if h:
                ...
                status = "✓" if (lines_count == 6 and has_judgment and has_image) else "✗"
...
def test_hexagram_display():
    ...
    try:
        ...
        from core.hexagram import HexagramCalculator, Hexagram
        ...
        print(f"  符号：{original.display_unicode()}")
...
def test_history_function():
    ...
    try:
        ...
        if record:
            print(f"  读取成功：ID={record['id']}, 卦名={record['hexagram_name']}")
        else:
            print(f"  读取失败")
...
def test_hexagram_query():
    ...
    try:
        ...
        for hex_id in test_ids:
            h = next((x for x in hexagrams if x['id'] == hex_id), None)
            if h:
                print(f"  ✓ ID {hex_id:2d}: {h['name']} ({h['full_name']})")
            else:
                print(f"  ✗ ID {hex_id}: 未找到")
        ...
        for name in test_names:
            ...
            if h:
                print(f"  ✓ {name}: ID={h['id']}, 二进制={h['binary']}")
            else:
                print(f"  ✗ {name}: 未找到")
        ...
        for upper, lower in test_queries:
            ...
            if matches:
                for m in matches:
                    print(f"  ✓ 上{upper}下{lower}: {m['name']} (ID={m['id']})")
            else:
                print(f"  ✗ 上{upper}下{lower}: 未找到")
