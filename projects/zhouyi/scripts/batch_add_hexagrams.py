...
def main():
    data_file = os.path.join(os.path.dirname(__file__), '..', 'zhouyi', 'data', 'hexagrams.json')
    ...
    print(f"现有卦数：{len(data['hexagrams'])} 卦")
    print(f"现有卦 ID: {sorted(existing_ids)}")
    ...
    for hexagram in new_hexagrams:
        if hexagram['id'] not in existing_ids:
            ...
            added_count += 1
            print(f"✓ 添加：{hexagram['id']} {hexagram['name']}")
        else:
            print(f"⊘ 跳过（已存在）：{hexagram['id']} {hexagram['name']}")
    ...
    print(f"\n新增：{added_count} 卦")
    print(f"总计：{len(data['hexagrams'])} 卦")
    print(f"完成度：{len(data['hexagrams'])/64*100:.1f}%")
